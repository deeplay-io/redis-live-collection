...
import {removeVersionRange} from './removeVersionRange';
import {set} from './set';
...
test('removeVersionRange', async () => {
  ...
  const result1 = await removeVersionRange(redis, collection, '(inf', Infinity);
  ...
  await removeVersionRange(redis, collection, '(-inf', Infinity);
